...
if ${train_experts} ; then
    ...
    do
        if [ "${setup}" = "att" ]; then
            ...
            bsub -n 5 -W 4:00 -R "rusage[mem=6000,scratch=65000,ngpus_excl_p=1]" -K python train.py with caro_EEG_2D_no_sweat singlechanexp save_model=True ds.fold=${i} ds.data_dir=${data_path} ds.train_csv=${train_csv} ds.val_csv=${val_csv} log_dir=${log_dir}EEG_raw&
        fi
        if [ "${setup}" = "dssm" ]; then
           bsub -n 5 -W 10:00 -R "rusage[mem=8000,scratch=60000,ngpus_excl_p=1]" -K python train.py with ${data_config} DSSM_caro save_model=True unsupervized=${unsupervized} ds.fold=${i} ds.data_dir=${data_path} ds.train_csv=${train_csv} ds.val_csv=${val_csv} unsupervized=True log_dir=${log_dir}${setup}&
        fi
    done
    wait
fi
...
if ${train} ; then
    for (( i=0; i<${n_folds}; i++ ))
     do
        bsub -n 5 -W 10:00 -R "rusage[mem=16000,scratch=60000,ngpus_excl_p=1]" -K python train.py with ${data_config} ${model} save_model=True unsupervized=${unsupervized} ds.fold=${i} ds.data_dir=${data_path} ds.train_csv=${train_csv} ds.val_csv=${val_csv} log_dir=${log_dir}${setup}&
    done
    wait
fi
...
if ${cv} ; then
    bsub -n 5 -W 10:00 -R "rusage[mem=16000,scratch=60000,ngpus_excl_p=1]" -K python cv.py with ${data_config} ${model} save_model=True unsupervized=${unsupervized} ds.nfolds=${n_folds} ds.data_dir=${data_path} ds.train_csv=${train_csv} ds.val_csv=${test_csv} log_dir=${log_dir}${setup}&
    wait
fi
...
if ${evaluate} ; then
    ...
    for (( i=0; i<${n_folds}; i++ ))
    do
        bsub -n 5 -W 4:00 -R "rusage[mem=16000,scratch=60000,ngpus_excl_p=1]" -K python validate.py --model=${eval_file} --data_dir=${data_dir} --subject_csv=${test_csv} --output_dir=${out_dir}&
    done
    wait
fi
